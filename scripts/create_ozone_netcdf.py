...
def create_netcdf(ozone_config):
    ...
    if analysis_use:
        binned_data = spatial_bin(data[dict_key]['assimilated'],
                                  lats[dict_key]['assimilated'],
                                  lons[dict_key]['assimilated'],
                                  binsize='1x1')

        write_netcdf(data[dict_key]['assimilated'],
                     binned_data, metadata, outdir)

    else:
        binned_data = spatial_bin(
            data[dict_key], lats[dict_key], lons[dict_key], binsize='1x1')
