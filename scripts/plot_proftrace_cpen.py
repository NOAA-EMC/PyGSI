...
def collect_statistics(setdict):
    ######################################################################
    #
    # Collect penalty and ob-count statistics for each data (sub)set
    ...
    # (sub)set is present, the penalty and ob-count are set to zero but
    ...
    # DEPENDENCIES:
    #    os
    #    pyGSI.gsi_stat_pen.GSIstat # Modified, retains penalty data
    #    pandas
    ######################################################################
    import os
    from pyGSI.gsi_stat_pen import GSIstat
    import pandas
    ...
    for sd in setdict:
        ...
        for cycle in cycles:
            ...
            try:
                ...
                # (index=[0:3] is count and penalty of first entry)
                s = s.iloc[0:3, :]
    ...
    return cpens, counts, levels


def aggregate_figure_data(cpens, counts, levs):
    ######################################################################
    #
    # Generate plotting data by aggregating penalty and count data
    ...
    #
    # INPUTS:
    #    cpens: Dictionary of penalty data for each subset
    ...
    for setname in cpens.keys():
        ...
        cpen = np.zeros((nlev, ))
        nobs = np.zeros((nlev, ))
        dates = list(cpens[setname].keys())
        date_trace.append(dates)
        ndate = len(dates)
        p_trace = np.zeros((ndate, ))
        n_trace = np.zeros((ndate, ))
        for i in range(ndate):
            date = dates[i]
            n_ele = np.size(cpens[setname][date]['col'].values)
            for j in range(n_ele):
                # Aggregate penalty and count for full-column values at each
                ...
                p_trace[i] = pcol2
                n_trace[i] = ncol2
                # Aggregate penalty and count at each level across all times
                # to define profiles
                p = cpens[setname][date]['lev'].values[j]
    ...
    # Return output lists
    return (cpen_profs, cpen_trace, nobs_profs,
            nobs_trace, levl_profs, date_trace, name_list)


def plot_stat_profiles(penList, countList, nameList, plevList,
                       colMap=['tab10', 10]):
    ######################################################################
...
def plot_stat_traces(penList, countList, nameList, dateList,
...
if __name__ == "__main__":
    ...
    #
    # If a gsistat file exists for a given cycle but no data for a data
    # (sub)set is present, the penalty and ob-count are set to zero
    # but the cycle is not skipped. Statistics with zero ob-count are
    # reassigned to NaN later.
    #
    cpens, counts, levels = collect_statistics(setdict)
    ######################################################################
    #
    # Generate plotting data by aggregating penalty and count data
    # across times (for profiles), or aggregate full-column data (for
    # traces)
    #
    (cpen_profs, cpen_trace,
     nobs_profs, nobs_trace, levl_profs, date_trace,
     name_list) = aggregate_figure_data(cpens, counts, levels)
    ######################################################################
    #
    # Generate profile plot
    #
    fig_prof = plot_stat_profiles(cpen_profs, nobs_profs,
    ...
    # Generate trace plot
    #
    fig_trace = plot_stat_traces(cpen_trace, nobs_trace,
