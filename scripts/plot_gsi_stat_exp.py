...
def gen_figure(datadict, datatypestr, labels, sdate, edate, save, plotdir):
    ...
    for v, var in enumerate(obvars):
        ...
        for e,expid in enumerate(labels):
            ...
            ax.plot(profile,levels[:-1],marker='o',color=mc[e],mfc=mc[e],mec=mc[e],label=labels[e])
            if ( var in ['q'] ):
    ...
    plt.figtext(0.5,0.93,'%s O-F (%s-%s)'%(datatypestr,sdatestr,edatestr),horizontalalignment='center',fontsize=18)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g','--gsistats',help='list of directories containing GSI stat files',nargs='+',required=True)
    parser.add_argument('-l','--label',help='list of labels for experiment IDs',nargs='+',required=False)
    parser.add_argument('-f','--save_figure',help='save figures as png and pdf',action='store_true',required=False)
    parser.add_argument('-p','--plotdir',help='path to where to save figures',default='./',required=False)
    ...
    for exp, gsistats in zip(labels, args.gsistats):
        ...
        for gsistat, cycle in zip(statfiles, cycles):
            ...
            try:
                ...
                print('Error: One or more expected gsistat files missing, check dates and paths!')
                sys.exit(1)
            ...
            for var in obvars:
                ...
                rmses[exp][cycle][var] = tmpstat[tmpstat.index.isin(['rms'],level='stat')]
                counts[exp][cycle][var] = tmpstat[tmpstat.index.isin(['count'],level='stat')]
                biases[exp][cycle][var] = tmpstat[tmpstat.index.isin(['bias'],level='stat')]

    # now aggregate stats
    for exp in labels:
        rmses[exp]['mean'] = {}; biases[exp]['mean'] = {}; counts[exp]['sum'] = {}
        for var in obvars:
    ...
    gen_figure(counts, 'Count', labels, sdate, edate, save_figure, args.plotdir)
