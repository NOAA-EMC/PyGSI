...
def binData(data, lat, lon, binsize='1x1', uvData=False, pressure=None):
    """
    ...
    Inputs:
        ...
        uvData    : if using uvData, will be True (Default = False)
...
def spatialBin(data, lat, lon, binsize='1x1', uvData=False, pressure=None, pbins=None):
    """
    ...
    if uvData == True:
        if pressure is None:
            binned_u_data, binned_v_data = binData(data, lat, lon,
                                                   binsize=binsize,
                                                   uvData=uvData,
                                                   pressure=pressure)
            ...
            for x in range(0, rows):
                for y in range(0, cols):
                    if np.isnan(binned_u_data[x,y]).any() == False:
                        ...
                        binned_u_rmse[x,y] = np.sqrt(np.nanmean(np.square(binned_u_data[x,y])))
                        ...
                        binned_v_rmse[x,y] = np.sqrt(np.nanmean(np.square(binned_v_data[x,y])))           
